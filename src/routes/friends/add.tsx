  ...
  return (
    <div className="items-center justify-center flex flex-col p-4 container mx-auto gap-4">
      ...
      <div className='w-full'>
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 grid-rows-auto gap-4">
          {filteredUsers.map(user => <UserCard key={user.id} user={user} isSelected={false} onToggle={() => toggleUserSelection(user.id)} />)}
        </div>
      </div>
    </div>
  );
...
function UserCard({ user, isSelected, onToggle }: { user: any; isSelected: boolean; onToggle: () => void }) {
  return (
